...
class AutoEncoder_2D(tf.keras.Model):
    ...
    def __init__(self, encoder, decoder, loss_function=tf.keras.losses.MAE, *args, **kwargs):
        super().__init__()
    ...
    @tf.function(experimental_compile=True)
    def get_loss(self, data, training=True, mask=None):
        encoded = self.encoder(data)
        decoded = self.decoder(encoded)
    ...
    def train_step(self, input_data):
        with tf.GradientTape() as tape:
            ...
            # reconstruction_loss = self.loss_function(input_data, self.call(input_data))
...
def make_encoder(in_shape):
    ...
    x = tf.keras.layers.Convolution1D(filters=32, kernel_size=2, strides=2, kernel_regularizer=regularizer)(encoder_input)
...
def make_decoder(in_shape):
    ...
    x = tf.keras.layers.Conv1DTranspose(filters=in_shape[1], kernel_size=3, strides=3, kernel_regularizer=regularizer)(x)
    decoder_output = decoder_output_activation(x)

    decoder: tf.keras.Model = tf.keras.Model(inputs=decoder_input, outputs=decoder_output)
    return decoder
